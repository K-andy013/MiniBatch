...
class DataGenValidation:
    ...
    def __init__(self, path, split_ratio, x, y, color_space='rgb'):
        ...
        self.path_train_images = path + "images/"
        self.path_train_labels = path + "labels/"
        self.path_test_images = path + "images/"
        self.path_test_labels = path + "labels/"
            ...
            for b in range(batch_size):
                ...
                if i < len(image_file_list):
                    ...
                    if train or val:
                        ...
                        image = cv2.resize(image, (224,224)) # I inserted
                        label = cv2.imread(self.path_train_labels + sample_label_filename, 0)
                        label = cv2.resize(label, (224,224))# I inserted
                    elif test is True:
...
def load_data(path):
    ...
    path_train_images = path + "images/"
    path_train_labels = path + "labels/"
    path_test_images = path + "images/"
    path_test_labels = path + "labels/"
...
def save_results(np_array, color_space, outpath, test_label_filenames_list, threshold=0.5):
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    for i, filename in enumerate(test_label_filenames_list):
        pred = np_array[i]
        # Binarize the prediction for black and white masks
